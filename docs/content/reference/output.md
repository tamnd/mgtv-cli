...
describes what users get. Pick a format with `-o`, or let mgtv choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mgtv <command> -o table   # aligned columns for reading
mgtv <command> -o jsonl   # one JSON object per line, for piping
mgtv <command> -o json    # a single JSON array
mgtv <command> -o csv     # spreadsheet friendly
mgtv <command> -o tsv     # tab-separated
mgtv <command> -o url     # just the URL column
mgtv <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mgtv <command> --fields id,title,url
```
...
```bash
mgtv <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mgtv <command>            # a table, because this is a terminal
mgtv <command> | wc -l    # JSONL, because this is a pipe
```
